...
@external
def deploy(_masterCopy: address, owner: address, _funding_goal:uint256,
                _funding_time:uint256, _beneficiary:address, _milestone:uint256, 
                _time:uint256):
    addr: address = create_forwarder_to(_masterCopy)
    ContractToDeploy(addr).setup(_masterCopy, owner, _funding_goal,_funding_time,
                _beneficiary,_milestone,_time)
    log DeployLog(_masterCopy, owner, _funding_goal, _funding_time, 
                _beneficiary, _milestone, _time)

@external
def deployTest(_masterCopy: address, owner: address,_funding_goal:uint256,
        _funding_time:uint256, _beneficiary:address, _milestone:uint256,
        _time:uint256):
    addr: address = create_forwarder_to(_masterCopy)
    ContractToDeploy(addr).setup(_masterCopy, owner, _funding_goal,
