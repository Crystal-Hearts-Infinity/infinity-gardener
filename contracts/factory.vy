...
event DeployLog:
    ...
    owner: indexed(address) #owner of the Smart Contracts, it belongs to the corresponding Charity Entity
    ...
    milestone: int128 #milestones for the beneficiaty to achieve. for example, a number of tokens representing the contribution of 
...
@notice please refer https://vyper-by-example.org/create-new-contract/
"""
def deploy(_masterCopy: address, owner: address,_funding_goal:uint256, _funding_time:uint256, _beneficiary:address,_milestone:uint256,_time:uint256):
...
@external
def deployTest(_masterCopy: address, owner: address,_funding_goal:uint256, _funding_time:uint256, _beneficiary:address,_milestone:uint256,_time:uint256):
    addr: address = create_forwarder_to(_masterCopy)
